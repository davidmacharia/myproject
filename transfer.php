...
$item = "SELECT buyerDetails.*, documents.* 
         FROM buyerDetails 
         JOIN documents ON buyerDetails.ownerId = documents.documentId
         WHERE buyerDetails.Email = ?";
$stmt_item = $connect->prepare($item);
if (!$stmt_item) {
    die("Prepare statement failed: " . $connect->error);
}
$stmt_item->bind_param("s", $email);
$stmt_item->execute();
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $previousdeed = $row['titleDeed']; // Overwrite if data exists
    }
}
$stmt_item->close();
$stmt_sale->close();
...
<div class="container">
    <div class="card">
        <div class="card-header">
            Pending Land Registration Transfers
        </div>
        <div class="card-body">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            <?php while ($row = $allOwnerships->fetch_assoc()): ?>
                                <tr>
                                    <td><?php echo htmlspecialchars($row['FirstName']."   ".$row['FirstName']); ?></td>
